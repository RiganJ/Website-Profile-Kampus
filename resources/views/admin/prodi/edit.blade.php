...
<div class="card">
    <div class="card-body">
        <form method="POST" action="/admin/prodi/{{ $prodi->id }}" enctype="multipart/form-data"
              ...
              data-confirm-title="Update Prodi?"
              data-confirm-text="Perubahan data prodi akan disimpan."
              data-confirm-button="Ya, update">
            @csrf
            @method('PUT')
            <div class="mb-3"><label>Kode Prodi</label><input type="text" name="kode_prodi" class="form-control" value="{{ old('kode_prodi', $prodi->kode_prodi) }}" required></div>
            <div class="mb-3">
                <label>Nama Prodi</label>
                <select name="nama_prodi" class="form-control" required>
                    @foreach($namaProdiOptions as $namaProdi)
                        <option value="{{ $namaProdi }}" {{ old('nama_prodi', $prodi->nama_prodi) === $namaProdi ? 'selected' : '' }}>
                            {{ $namaProdi }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-3">
                ...
                <select name="kaprodi_dosen_id"
                    ...
                    @foreach($dosenOptions as $dosen)
                        <option value="{{ $dosen->id }}" {{ old('kaprodi_dosen_id', $prodi->kaprodi_dosen_id) == $dosen->id ? 'selected' : '' }}>
                            {{ $dosen->nama }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-3">
                ...
                <select name="admin_prodi_civitas_id"
                    ...
                    @foreach($civitasOptions as $civitas)
                        <option value="{{ $civitas->id }}" {{ old('admin_prodi_civitas_id', $prodi->admin_prodi_civitas_id) == $civitas->id ? 'selected' : '' }}>
                            {{ $civitas->nama }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-3">
                ...
                <select name="laboran_civitas_ids[]"
                        ...
                        multiple>
                    @foreach($civitasOptions as $civitas)
                        <option value="{{ $civitas->id }}" {{ in_array($civitas->id, old('laboran_civitas_ids', $prodi->laborans->pluck('id')->all())) ? 'selected' : '' }}>
            ...
            <div class="mb-4"><label>Fakultas</label><select name="fakultas_id" class="form-control" required>@foreach($fakultas as $f)<option value="{{ $f->id }}" {{ old('fakultas_id', $prodi->fakultas_id) == $f->id ? 'selected' : '' }}>{{ $f->nama_fakultas }}</option>@endforeach</select></div>
            <div class="border rounded p-3 mb-4">
                <h5 class="mb-3">Hero Halaman Prodi</h5>
                <div class="mb-3">
                    <label>Judul Hero</label>
                    <input type="text" name="hero_title" class="form-control" value="{{ old('hero_title', $prodi->hero_title) }}" placeholder="Kosongkan untuk memakai judul bawaan">
                    <small class="prodi-help-text">Judul ini akan tampil di bagian hero halaman prodi.</small>
                </div>
                <div class="mb-3">
                    <label>Deskripsi Hero</label>
                    <textarea name="hero_subtitle" class="form-control" rows="3" placeholder="Kosongkan untuk memakai deskripsi bawaan">{{ old('hero_subtitle', $prodi->hero_subtitle) }}</textarea>
                </div>
                <div class="mb-3">
                    <label>Gambar Hero</label>
                    @if($prodi->hero_image_url)
                        <div class="mb-2">
                            <img src="{{ $prodi->hero_image_url }}" alt="Gambar hero {{ $prodi->nama_prodi }}" width="180" height="90" style="object-fit:cover;border-radius:8px;">
                        </div>
                    ...
                    <small class="prodi-help-text">Upload gambar baru untuk mengganti gambar hero. Format jpg, jpeg, png, atau webp. Maksimal 4 MB.</small>
                </div>
                <div class="mb-0">
            ...
            @php
                $selectedDosenIds = collect(old('dosen_ids', $prodi->dosen->pluck('id')->all()))->map(fn($id) => (int) $id)->all();
            @endphp
            <div class="mb-4">
                ...
                <select name="dosen_ids[]"
                    ...
                    @foreach($dosenOptions as $dosen)
                        <option value="{{ $dosen->id }}" {{ in_array($dosen->id, $selectedDosenIds, true) ? 'selected' : '' }}>
                            {{ $dosen->nama }}
                        </option>
                    @endforeach
                </select>
                <small class="prodi-help-text">Ubah pilihan untuk menambah atau menghapus dosen pengajar pada prodi ini.</small>
            </div>
            <div class="d-flex gap-2">
                <button class="prodi-form-btn prodi-form-btn--save" title="Update Prodi" aria-label="Update Prodi"><i class="fas fa-save"></i></button>
