...
## Aturan Asset dan Git

Foto dan media runtime berada di [`public/images/`](./public/images/) dan sengaja tidak dibawa oleh Git melalui aturan berikut:
...
- Foto lokal tidak muncul sebagai perubahan Git.
- Foto harus disediakan melalui proses deployment, backup asset, atau storage terpisah.
- File source CSS/JS tetap berada di `resources/` dan dapat dibuild ulang.
- Asset hasil build berada di `public/build/`.
- File gambar yang sudah pernah ter-track sebelumnya tetap perlu dihapus dari index Git secara terpisah jika memang ingin mengeluarkannya dari repository.

## Keamanan

README ini tidak menyimpan akun, password, token, API key, application key, alamat layanan privat, atau data pengguna.

Jaga aturan berikut:

- Jangan commit `.env`, database produksi, dump database, log, backup, atau file upload sensitif.
- Jangan menulis kredensial ke source code, dokumentasi, issue, screenshot, atau chat.
- Gunakan `APP_DEBUG=false` di production.
- Gunakan HTTPS dan arahkan document root hanya ke `public/`.
- Batasi permission file dan direktori runtime.
- Sanitasi log, export, dan screenshot sebelum dibagikan.
- Rotasi secret jika pernah terekspos.
- Tinjau ulang role dan middleware ketika menambah modul admin baru.

## Kontribusi

1. Buat perubahan kecil dan terfokus.
2. Ikuti struktur Laravel yang sudah ada.
3. Jalankan test atau validasi yang relevan.
4. Jalankan `git diff --check`.
5. Pastikan tidak ada secret atau data sensitif dalam perubahan.

## Lisensi

Lisensi mengikuti kebijakan proyek dan lisensi dependensi yang digunakan. Pastikan lisensi asset pihak ketiga tetap dipatuhi saat distribusi dan deployment.
